...
export function drainPrompt(feedId: string, threadId: string): string {
  return [
    ...
    "For repo_execution work, the feed thread is an orchestrator only: reserve, create or reuse one exact repo-scoped Codex task, bind it, and monitor its structured receipt. Do not implement the material change in the feed thread.",
