...
| Hold work that needs no current judgment | Use the `Waiting` or `Blocked` tab; held cards expose `Check now` / `Resolve blocker`, `Retry` when applicable, `Move to review`, and `Dismiss` without exposing stale configured actions | `card:evaluate-triggers`, evidence-only queued checks, `work:block`, `work:retry`, and `card:return-to-review` preserve the same card and work identities |
| Delegate approved material repository work | Press the card's exact repo-delegation action | `delegate_repo_task` queues `repo_execution`; the home operator reserves and binds one exact repo task, that task claims the work, and `work:complete` validates its structured receipt before card completion |
